...
KvsClientInterface *client = &mock_cl;
...
public:
  ...
  construct_causal_lattice(const VectorClock &vc,
                           ...
                           const string &val) {
    MultiKeyCausalPayload<SetLattice<string>> ccp;
    ccp.vector_clock = vc;
    for (const auto &pair : dep) {
      ccp.dependencies.insert(pair.first, pair.second);
    }
    ccp.value.insert(val);

    return std::make_shared<MultiKeyCausalLattice<SetLattice<string>>>(ccp);
  }
};
