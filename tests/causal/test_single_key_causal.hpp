...
TEST_F(CausalCacheTest, SinglePartiallyCovered) {
  ...
  map<Key, set<Address>> expected_single_key_callback_map = {
      {"b", {dummy_address}}};

  map<Address, PendingClientMetadata> expected_pending_single_key_metadata = {
      {dummy_address, PendingClientMetadata("test", {"a", "b"}, {"b"})}};

  EXPECT_THAT(
      pending_single_key_metadata,
      testing::UnorderedElementsAreArray(expected_pending_single_key_metadata));
  EXPECT_THAT(single_key_callback_map, testing::UnorderedElementsAreArray(
                                           expected_single_key_callback_map));
...
TEST_F(CausalCacheTest, SingleEndToEnd) {
  ...
  for (const CausalTuple &tp : causal_response.tuples()) {
    ...
    values.push_back(deserialize_multi_key_causal(tp.payload()).values(0));
  }

  EXPECT_THAT(keys, testing::UnorderedElementsAreArray({"a", "b"}));
  EXPECT_THAT(values, testing::UnorderedElementsAreArray({value, value}));

  expected_pending_single_key_metadata.clear();
  expected_single_key_callback_map.clear();
