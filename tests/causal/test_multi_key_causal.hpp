...
TEST_F(CausalCacheTest, MultiKeyFullyCovered) {
  ...
  request.set_consistency(ConsistencyType::MULTI);
  request.set_id("test");
  // add two keys
  ...
  for (const CausalTuple &tp : response.tuples()) {
    ...
    values_result.push_back(
        deserialize_multi_key_causal(tp.payload()).values(0));
  }
...
TEST_F(CausalCacheTest, MultiKeyCoveredInUnmerged) {
...
TEST_F(CausalCacheTest, MultiKeyCoveredInPreparation) {
  Key key = "a";
  VectorClock vc = construct_vector_clock({{"c1", 1}, {"c2", 1}});
  map<Key, VectorClock> dep;
...
TEST_F(CausalCacheTest, MultiKeyNotCovered) {
  // issue GET request
  CausalRequest request;
  request.set_consistency(ConsistencyType::MULTI);
  request.set_id("test");
...
TEST_F(CausalCacheTest, MultiKeyDepNotCovered) {
  Key key = "a";
  VectorClock vc = construct_vector_clock({{"c1", 1}, {"c2", 1}});
  map<Key, VectorClock> dep;
  dep["b"] = construct_vector_clock({{"c1", 3}, {"c2", 2}});
...
TEST_F(CausalCacheTest, MultiKeyQuerySameKey) {
  Key key = "a";
  VectorClock vc = construct_vector_clock({{"c1", 1}, {"c2", 1}});
  map<Key, VectorClock> dep;
  dep["b"] = construct_vector_clock({{"c1", 3}, {"c2", 2}});
  ...
  map<Address, PendingClientMetadata> expected_pending_multi_key_metadata = {
      {dummy_address + "1", PendingClientMetadata("test1", {"a"}, {"a"})},
      {dummy_address + "2", PendingClientMetadata("test2", {"a"}, {"a"})}};

  EXPECT_THAT(
      pending_multi_key_metadata,
      testing::UnorderedElementsAreArray(expected_pending_multi_key_metadata));
